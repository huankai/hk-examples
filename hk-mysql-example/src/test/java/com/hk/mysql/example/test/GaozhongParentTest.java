...
/**
 * 非学业测试修改3稿\【3】高中二年级\【2】高中家长问卷
 *
 * @author huangkai
 * @date 2019-02-14 11:19
 */
@SpringBootTest(classes = {MysqlExampleApplication.class})
public class GaozhongParentTest extends BaseTest {
    ...
    public void quotaTest() {
        ...
        ReadResult<ScoringRules> readResult = readableExcel.read(new File("C:/Users/sjq-278/Desktop/非学业测试修改3稿/【3】高中二年级/【2】高中家长问卷/高中家长问卷计分规则.xlsx"));
        List<ScoringRules> data = readResult.getAllSheetData();
        String questionnaireId = "4d596df9-9bce-4879-b86b-802db0f12241";
        ...
        for (ScoringRules item : data) {
            ...
            if (questionCodeMap.isPresent()) {
                ...
                arguments.getConditions().addCondition(new SimpleCondition("parent_id", "6c6c60fc-c5cb-42b2-9464-ff08d50cb025"));
    ...
    @Test
    public void gaozhongParent() throws IOException {
        ReadParam<ScoringRules> readParam = ReadParam.<ScoringRules>builder()
                .beanClazz(ScoringRules.class)
                .build();
        ReadableExcel<ScoringRules> readableExcel = new DomReadExcel<>(readParam);
        ...
        FileInputStream fileInputStream = new FileInputStream("C:/Users/sjq-278/Desktop/非学业测试修改3稿/【3】高中二年级/【2】高中家长问卷/1【问卷测试题】高中家长问卷.docx");
...
//            System.out.println("text: " + text);
            if (StringUtils.isNotEmpty(text) && !StringUtils.startWithNumber(text)) {
                if (!StringUtils.startWithLetter(text)) {
                    ...
                    if (questionCode == 2) {
                        question.setQuestionType("10"); // 第2 题是多选
                    }
    ...
    private static final String QUESTIONNAIRE_ID = "4d596df9-9bce-4879-b86b-802db0f12241";
